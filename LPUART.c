...
// }
void LPUART1_init(void)  /* Init. summary: 31250 baud, 1 stop bit, 8 bit format, no parity */
{
  PCC->PCCn[PCC_LPUART1_INDEX] &= ~PCC_PCCn_CGC_MASK;    /* Ensure clk disabled for config */
  PCC->PCCn[PCC_LPUART1_INDEX] |= PCC_PCCn_PCS(0x001)    /* Clock Src= 1 (SOSCDIV2_CLK) */
                               |  PCC_PCCn_CGC_MASK;     /* Enable clock for LPUART1 regs */

  LPUART1->BAUD = (0xF << LPUART_BAUD_OSR_SHIFT) |  /* OSR = 15 */
                  (0x10 << LPUART_BAUD_SBR_SHIFT);  /* SBR = 16 */
                               /* Initialize for 31,250 baud, 1 stop bit */
                               /* OSR=15: Over sampling ratio = 15+1=16 */
                               /* SBR=16 (0x10): baud divisor = 8M/31250/16 = ~16 */
                               /* SBNS=0: One stop bit */
...
void LPUART1_transmit_int(int n) {
  if (n == 0) {
  ...
  for (int j = i - 1; j >= 0; j--) {
    LPUART1_transmit_char(buffer[j]);
  }
}
